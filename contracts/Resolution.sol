...
contract Resolution {
    ...
    )
  {
    ...
    for (uint256 i = 0; i < voters.length; i++) {
      // if it voted
      address voter = voters[i];
      address delegate = voting.getDelegateAt(voter, resolution.snapshotId);
  ...
  function _updateDelegateVotes(
    ...
    resolution.votes[delegate] = resolution.votes[delegate] - delegatorVotes;
  ...
  function vote(bool preference, uint256 resolutionId) public {
    ...
    address delegate = voting.getDelegateAt(msg.sender, resolution.snapshotId);
